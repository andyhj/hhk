...
class GyfCallbackController extends InitController {
    private $key = "db7770445fdde3a663f6b93d7a08382a";
    ...
    function receive(){
    	//接受请求的数据
        $data = I('post.');
		add_log("receive.log", "gyfpay", "订单异步返回信息：" . var_export($data, true));		
		$key = $this->key;
		$sign = md5($data['orderId'].$data['respCode'].$key);
        if($data['sign']==$sign){//验签成功
            ...
            if(!$plan_des_info&&empty($plan_des_info)){
                add_log("receive.log", "callback", "订单不存在：". var_export($order_id, true));
                die('SUCCESS');
            }
            // 如果订单状态成功
            if($plan_des_info&&!empty($plan_des_info)&&$plan_des_info["order_state"]==1){
                add_log("receive.log", "callback", "已付款成功：". var_export($plan_des_info, true));
                die('SUCCESS');
            }
            if($plan_des_info&&!empty($plan_des_info)&&$plan_des_info["order_state"]==2){
                add_log("receive.log", "callback", "待执行：". var_export($plan_des_info, true));
                die();
            }
            ...
            if($data['status'] == '02' ){
				...
				add_log("receive.log", "gyfpay", "处理中" );		
				die();
			}
      	}else{
			add_log("receive.log", "gyfpay", "验签失败" );		
			die();
      	}
		die();
	}
	...
	*/
    function close(){
		//接受请求的数据
        $data = I('post.');
		add_log("close.log", "gyfpay", "订单异步返回信息：" . var_export($data, true));		
		$key = $this->key;
		$sign = md5($data['orderId'].$data['respCode'].$key);
		if($data['sign']==$sign){//验签成功
			$order_id = $data['orderId'];
            ...
            $plan_des_info = $plan_des_model->where("order_id='{$order_id}' OR remedy_id='{$order_id}'")->find();
            if(!$plan_des_info&&empty($plan_des_info)){
                add_log("close.log", "callback", "订单不存在：". var_export($order_id, true));
                die('SUCCESS');
            }
            // 如果订单状态成功
            if($plan_des_info&&!empty($plan_des_info)&&$plan_des_info["order_state"]==1){
                add_log("close.log", "callback", "已付款成功：". var_export($plan_des_info, true));
                die('SUCCESS');
            }
            if($plan_des_info&&!empty($plan_des_info)&&$plan_des_info["order_state"]==2){
                add_log("close.log", "callback", "待执行：". var_export($plan_des_info, true));
                die();
            }
            $plan_info = $plan_model->where(["id"=>$plan_des_info["p_id"]])->find();
            if($data['status'] == '02' ){
                ...
                add_log("close.log", "gyfpay", "处理中" );		
				die();
			}
      	}else{
			add_log("close.log", "gyfpay", "验签失败" );		
			die();
      	}
		die();
    }
	...
	function bKreceive(){
    	//接受请求的数据
        $data = I('post.');
		add_log("bKreceive.log", "gyfpay", "订单异步返回信息：" . var_export($data, true));		
		$key = $this->key;
		$sign = md5($data['orderId'].$data['respCode'].$key);
        if($data['sign']==$sign){//验签成功
            if($data['respCode'] == '0000' && $data['cardId']){
                $bank_card_gyf_model = M("bank_card_gyf");
                $bank_card_gyf = $bank_card_gyf_model->where(["card_no"=>$data['cardId']])->save(["success"=>1]);
                if ($bank_card_gyf){                   
					die("SUCCESS");
                }else {
					add_log("bKreceive.log", "gyfpay", "更新信用卡失败：" . var_export($bank_card_gyf, true));		
                    die("SUCCESS");
                }
			}			
      	}else{
			add_log("bKreceive.log", "gyfpay", "验签失败，key：".$key );		
			die();
      	}
		die();
    }
