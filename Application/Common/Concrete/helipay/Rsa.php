...
class Rsa {
    public function __construct() {
        ...
        define('filePath', str_replace('\\', '/', realpath(dirname(__FILE__) . '/')) . "/cert/");
        /* 实现.pfx文件转为.pem文件 */
//        $file = filePath . pfxFileName;
//        $results = [];
//        add_log("helipay.log", "helipay", "pfxFileName：". var_export($file, true));
...
//        file_put_contents($certificateCApem, $certificateCAcerContent);
    ...
    public function curlPost($url,$data){
        ...
        add_log("helipay.log", "helipay", "请求response：". var_export($response, true));
        if (curl_errno($ch)) {
            add_log("helipay.log", "helipay", "请求curl_errno：". var_export($ch, true));
            return false; 
        }
        curl_close($ch);
        return $response;
