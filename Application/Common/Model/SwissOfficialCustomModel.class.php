...
use Think\Model;
class SwissOfficialCustomModel extends Model{
    protected $db_config;
    ...
    public function getOne($where){
        ...
        $model = M("swiss_official_custom",$this->db_config["DB_PREFIX"],$this->db_config);
    ...
    public function getApplyOne($where){
        $model = M("swiss_official_custom_apply",$this->db_config["DB_PREFIX"],$this->db_config);
        return $model->where($where)->find();
    }
    /**
     * 根据条件查询多条数据
     ...
     * @param type $cid
     */
    public function getApplyCount($cid){
        $model = M("swiss_official_custom_apply",$this->db_config["DB_PREFIX"],$this->db_config);
        $sql = "SELECT COUNT(DISTINCT uid) num FROM tb_swiss_official_custom_apply WHERE gameid=".$cid;
        return $model->query($sql);
    }
    /**
     * 添加一条数据
    ...
    public function getCustomRanking($where,$order="ranking ASC"){
        ...
        $model = M("swiss_official_custom_ranking",$this->db_config["DB_PREFIX"],$this->db_config);
        return $model->where($where)->order($order)->select();
    }
    /**
     * 获取一条比赛排名
     ...
     * @param type $sql
     */
    public function getRankingAllBySql($sql){
        $model = M("swiss_official_custom_ranking_all",$this->db_config["DB_PREFIX"],$this->db_config);
        return $model->query($sql);
    }
}
