...
class AccessModel extends Model {
    ...
    public function roleList() {
        $M = M("Role");
        $list = $M->where(["id"=>array('NEQ',1)])->select();
        foreach ($list as $k => $v) {
    ...
    public function adminList() {
        $list = M("Admin")->where(["aid"=>array('NEQ',1)])->select();
        foreach ($list as $k => $v) {
            $list[$k]['statusTxt'] = $v['status'] == 1 ? "启用" : "禁用";
        }
        return $list;
    ...
    public function editAdmin() {
        $M = M("admin");
        if (!empty($_POST['pwd'])) {
            $_POST['pwd'] = md5(trim($_POST['email']).trim($_POST['pwd']));
        } else {
            unset($_POST['pwd']);
        }
        $user_id = (int) $_POST['aid'];
        $role_id = (int) $_POST['role_id'];
        $roleStatus = M("role_user")->where("`user_id`=$user_id")->save(array('role_id' => $role_id));
        if ($M->save($_POST)) {
//            , 'url' => U("Access/index")
