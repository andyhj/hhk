...
class AccessController extends CommonController {
    ...
    public function index() {
        $data = D("Access")->adminList();
        $a_data=[];
        if($data){
            foreach ($data as $value) {
                $aid = $value["aid"];
                $role_user_info = M("role_user")->where(["user_id"=>$aid])->find();
                $value["role_name"] = "";
                if($role_user_info){
                    $role_info = M("role")->where(["id"=>$role_user_info["role_id"]])->find();
                    $value["role_name"] = $role_info["name"];
                }
                $a_data[] = $value;
            }
        }
        $this->assign("list",$a_data);
        $this->display();
    }
    ...
    public function changerole() {
        ...
        if (IS_POST) {
            ...
            $access = M("Access")->field("CONCAT(`node_id`,':',`level`,':',`pid`,':',`module`) as val")->where("`role_id`=" . $info['id'])->select();
            $admin_info = $_SESSION['my_info'];
            $role_user_info = M("role_user")->where(["user_id"=>$admin_info["aid"]])->find();
            
            $we = "";
            $wein = "";
            if($role_user_info){
                $access_list = M("Access")->where(["role_id"=>$role_user_info["role_id"]])->select();
                if($access_list){
                    foreach ($access_list as $ae) {
                        $wein .= $ae["node_id"].",";
                    }
                    $wein = rtrim($wein, ','); 
                    if($wein){
            ...
            $datas = $M->where("level=1 $we")->select();
            foreach ($datas as $k => $v) {
                $map['level'] = 2;
                $map['pid'] = $v['id'];
                if($wein){
                    $map['id'] = array('in',$wein);
                }
