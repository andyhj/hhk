...
class SysDataController extends CommonController {
    ...
    public function index() {
        ...
        $total = 0;
        foreach ($tabs as $k => $v) {
            $tabs[$k]['size'] = format_price($v['data_length'] + $v['index_length']);
            $total+=$v['data_length'] + $v['index_length'];
        }
        $this->assign("list", $tabs);
        $this->assign("total", format_price($total));
...
//取得上次文件导入到sql的句柄位置
        ...
        foreach ($files as $fileKey => $sqlFile) {
            ...
            $file = fopen($file, "r");
//            fclose($file);
//            unset($_SESSION['cacheRestore']['files'][$fileKey]);die();
            $sql = "";
...
//检测一行字符串最后有个字符是否是分号，是分号则一条sql语句结束，否则sql还有一部分在下一行中
    ...
    public function delSqlFiles() {
        if (IS_POST) {
            ...
            echo json_encode(array("status" => 1, "info" => "已删除：" . implode("、", $files), "url" =>U("SysData/restore")));
        }
    }
    ...
    public function sendSql() {
        ...
        if (IS_POST) {
            ...
            foreach ($files as $k => $zipFiles) {
                ...
                if (D("SysData")->zip($zipFiles, $zipOut, DatabaseBackDir . "Zip/")) {
//发送邮件
                    //send_mail($to, "", "数据库备份{$k}/{$sum}", "网站：<b>" . $this->site['SITE_INFO']['name'] . "</b> 数据文件备份", WEB_CACHE_PATH . $zipOut);
                    sendMail($to, "数据库备份{$k}/{$sum}", "网站：<b>" . $this->site['SITE_INFO']['name'] . "</b> 数据文件备份","数据备份", DatabaseBackDir . "Zip/" . $zipOut);
                    delDirAndFile(DatabaseBackDir . "Zip/" . $zipOut); //删除已发送附件
    ...
    public function repair() {
        ...
        if (IS_POST) {
            ...
            foreach ($tabs as $k => $table) {
                $table['size'] = format_price($table['data_length'] + $table['index_length']);
                $totalsize['table'] += $table['data_length'] + $table['index_length'];
                $totalsize['data']+=$table['data_length'];
                $table['data_length'] = format_price($table['data_length']);
                $totalsize['index']+=$table['index_length'];
                $table['index_length'] = format_price($table['index_length']);
                $totalsize['free']+=$table['data_free'];
                $table['data_free'] = format_price($table['data_free']);
                $tables[] = $table;
            }
