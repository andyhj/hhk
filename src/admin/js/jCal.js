...
(function($) {
	...
	$.jCal = function (target, opt) {
		opt = $.extend({
			...
			dayOffset:		0,											// 0=周日开始, 1=周一开始
			forceWeek:		true,										// true=选择周, false=从每一天开始
			dow:			['日', '一', '二', '三', '四', '五', '六'],		// 星期
				...
				.bind('click', $.extend( {}, opt ),
					function (e) {
						...
						$('<div class="monthSelectorShadow" style="' +
							'top:' + ($(e.data._target).offset().top-55) + 'px; ' +
							'left:' + ($(e.data._target).offset().left-529.5) + 'px; ' +
						...
						$('<div class="monthSelector" style="' +
							'top:' + calcTop + 'px; ' +
							'left:' + ( ($(this).offset()).left-532.5 ) + 'px; ' +
				...
				(( d <= ( copt.fd - opt.dayOffset ) ) ? 
					...
					: ( ( d > ( ( copt.fd - opt.dayOffset ) + copt.ld ) ) ?
						...
						: '<div id="' + opt.cID + 'd_' + (fd.getMonth() + 1) + '_' + ( d - ( copt.fd - opt.dayOffset ) ) + '_' + fd.getFullYear() + '" class="' +
							( ( opt.dCheck( new Date( (new Date( fd.getTime() )).setDate( d - ( copt.fd - opt.dayOffset ) ) ) ) ) ? 'day' : 'day' ) +
